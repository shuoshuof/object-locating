...
class RandomTarget_dataset(tf.keras.utils.Sequence):
    ...
    #         x, y = random.randint(0, self.bg_w - self.box_r), random.randint(0,self.bg_r - self.box_r)
    #         while judge_point(img_pos, x, y) != True:
    #             x, y = random.randint(0, self.bg_w - self.box_r+1), random.randint(0, self.bg_r - self.box_r+1)
    #         img_pos.append([x, y])
    #     if self.Chinese_path:
    #         img = cv_imread(path)
    #     else:
    ...
    #     img0 = np.zeros((self.bg_r, self.bg_w, 3), dtype=np.uint8)#创建一张黑色画布
    #
    #     img_center_points = []#图片中心的偏移
    #     for point in img_pos:
    #         x, y = point
    #         size = random.randint(self.img_size[0],self.img_size[1])
    #
    #         img_rotate = random_rotate(img)
    #
    #         img_rotate = cv2.resize(img_rotate, (size, size))
    #         img0[y:y + size, x:x + size, :] = img_rotate[:, :, :]
    #         img_center_points.append([x + size // 2, y + size // 2])
    #
    #     mask = get_mask(img0)
    #     bg_img = cv2.bitwise_and(bg_img, bg_img, mask=mask)
    #     img1 = cv2.add(bg_img, img0)
    #     label = self.encode(img_center_points)
    #     """
    #     opencv 为bgr要转rgb
    #     """
    #     img1 = img1[:,:,::-1]
    ...
    def creat_random_data(self,path,bg_img_path):
        ...
        for i in range(img_num):
            ...
            t =0
            while judge_point(img_center_points, x, y,self.target_d) != True:
                x, y = random.randint(l, self.bg_w-1-l), random.randint(l, self.bg_r-1-l)
                t+=1
                if t>=50:
                    break
        ...
        bg_img = cv2.bitwise_and(bg_img, bg_img, mask=mask)
        img1 = cv2.add(bg_img, img0)
        label = self.encode(img_center_points,self.box_r)
        """
        opencv 为bgr要转rgb
        """
        img1 = img1[:,:,::-1]
        return np.array(img1,dtype=np.float32),np.array(label,dtype=np.float32)
    def encode(self,img_center_points,box_r):
