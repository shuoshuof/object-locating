...
class mobilenet_backbone():
    def __init__(self,
        ...
        self.filters_list = [24,32,64,96]#
        assert input_shape[0]//FeatureMap_shape[0] == input_shape[1]//FeatureMap_shape[1]
        self.downsampling_num = round(math.log2(input_shape[0]//FeatureMap_shape[0]))-1
    def __call__(self, inputs, *args, **kwargs):
        first_filters = _make_divisible(32 * self.alpha, 8)
        x = _conv_block(inputs, first_filters, (3, 3), strides=(2, 2))
        x = _inverted_residual_block(x, 16, (3, 3), t=1, alpha=self.alpha, strides=(1, 1), n=1)
        for i,filters in enumerate(self.filters_list[:self.downsampling_num]):
            x = _inverted_residual_block(x,filters, (3, 3), t=6, alpha=self.alpha, strides=(2,2), n=1)
        return x
